    ...
    class Global
    {
        ...
        public static void CheckUpdate()
        {
            try
            {
                ...
                {
                    // Console.Title = string.Format("检测到更新，版本：{0}! 正在尝试自动下载新版", latestVer);
                    try
                    {
                        //尝试下载新版本
                        ...
                        {
                            // Console.Title = string.Format("检测到更新，版本：{0}! 新版下载成功，请您自行替换", latestVer);
                            return;
                        }
                        ...
                        //if (File.Exists(Path.Combine(Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName), $"N_m3u8DL-CLI_v{latestVer}.exe")))
                            // Console.Title = string.Format("检测到更新，版本：{0}! 新版下载成功，请您自行替换", latestVer);
                        //else
                            // Console.Title = string.Format("检测到更新，版本：{0}! 新版下载失败，请您自行下载", latestVer);
                    }
                    catch (Exception)
                    {
                        ;
        ...
        public static JObject GetAdJson(string url)
        {
        ...
        // 通过网络 url 获取 AdJsonData
        public static string GetAdJsonData(){
            if (Properties.Settings.Default.AdJsonData == null || properties.Settings.Default.AdJsonData.Count == ""){
                JObject adJsonData = GetAdJson("https://m3u8.bt-mao.com/pages/main");
                if (adJsonData != null){
                    Properties.Settings.Default.AdJsonData = adJsonData.ToString();
                    Properties.Settings.Default.AdRequestTime = DateTime.Now;
                    Properties.Settings.Default.Save();
                }
            }

            // 如果 Properties AdRequestTime 是否大于 10 分钟，重新拉取数据
            if (Properties.Settings.Default.AdRequestTime > 10 * 60) {
                JObject adJsonData = GetAdJson("https://m3u8.bt-mao.com/pages/main");
                if (adJsonData != null){
                    Properties.Settings.Default.AdJsonData = adJsonData.ToString();
                    Properties.Settings.Default.AdRequestTime = DateTime.Now;
            ...
            // 返回 AdJsonData
            return Properties.Settings.Default.AdJsonData;
        }
    }
}
