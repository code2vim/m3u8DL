...
namespace m3u8DL
{
    class DownloadManager
    {
        ...
        public void DoDownload(ObservableCollection<Piece> pieceItems, System.Windows.Threading.Dispatcher dispatcher, System.Windows.Controls.ProgressBar pieceProgressBar, DownloadDetail downloadDetail)
        {
            ...
            try
            {
                if (initJson["m3u8Info"]["audio"] != null && initJson["m3u8Info"]["audio"].ToString() != "")
                {
                    externalAudio = true;
            ...
            try
            {
                if (initJson["m3u8Info"]["sub"] != null && initJson["m3u8Info"]["sub"].ToString() != "")
                {
                    externalSub = true;
            ...
            Watcher watcher = new Watcher(DownDir, dispatcher, pieceProgressBar, downloadDetail);
            watcher.Total = total;
        ...
        downloadMap:
            ...
            try
            {
                ParallelLoopResult result = Parallel.ForEach(segments,
                    ...
                    (info, loopstate, index, sd) =>
                    {
                        ...
                        pieceItems[(int)index + 1].Status = 2;
                        return sd;
                    },
                    (sd) => { });
            ...
            //检测是否下完
            IsComplete(Convert.ToInt32(segCount), pieceItems, dispatcher, pieceProgressBar, downloadDetail);
        }

        public void IsComplete(int segCount, ObservableCollection<Piece> pieceItems, System.Windows.Threading.Dispatcher dispatcher, System.Windows.Controls.ProgressBar pieceProgressBar, DownloadDetail downloadDetail)
        {
            int tsCount = 0;
        ...
        ll:
            if (tsCount != segCount)
            {
                ...
                if (Count <= RetryCount)
                {
                    ...
                    Thread.Sleep(3000);
                    DoDownload(pieceItems, dispatcher, pieceProgressBar, downloadDetail);
                }
            }
            else  //开始合并
            {
